...
public class CustomerReceivableServiceImpl implements CustomerReceivableService {
	...
	@Override
	public CustomerReceivableReport generateCustomerReceivableReport(Long customerId, long minDaysOver,
			long maxDaysOver) {
	...
	private CustomerReceivableReport generateCustomerReceivablesFromInvoices(Long customerId, long minDaysOver,
			long maxDaysOver) {
		List<Billable> list = billableService.findUnpaidBillables(customerId);
		if (list != null)
	...
	private void generateReceivableList(long min, long max, List<Billable> list) {
		receivables = list.stream().filter(b -> daysOver(b) >= min && daysOver(b) <= max)
