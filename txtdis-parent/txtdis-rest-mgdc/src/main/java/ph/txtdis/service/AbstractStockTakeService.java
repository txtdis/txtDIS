...
public abstract class AbstractStockTakeService implements StockTakeService {
	...
	@Override
	public StockTake findByDate(Date d) {
		StockTakeEntity i = findLatestEntityOnOrBeforeCutoff(d.toLocalDate());
		return toStockTake(i);
	}

	@Override
	public StockTake findByWarehouseAndDate(String s, Date d) {
		StockTakeEntity i = findByWarehouseAndDate(s, d.toLocalDate());
		return toStockTake(i);
	}

	private StockTakeEntity findByWarehouseAndDate(String s, LocalDate d) {
		if (s.equals("all"))
	...
	private int getQtyPerCase(StockTakeDetailEntity e) {
		if (e.getUom() != UomType.CS)
			return 0;
		return itemService.getQtyPerCase(e.getItem());
	}
	...
	private ItemEntity toItem(StockTakeDetail ad) {
		return itemService.findEntity(ad.getId());
	}
