...
import ph.txtdis.repository.WeeklyVisitRepository;

@Service("weeklyVisitService")
public class WeeklyVisitServiceImpl implements WeeklyVisitService {

	@Autowired
	private WeeklyVisitRepository repository;

	@Override
	public List<WeeklyVisit> toList(List<WeeklyVisitEntity> l) {
		return l == null ? null : l.stream().map(e -> convert(e)).collect(Collectors.toList());
	}

	private WeeklyVisit convert(WeeklyVisitEntity e) {
