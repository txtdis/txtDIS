...
public class Revision2Impl //
	...
	private List<RoutingEntity> routings(CustomerEntity customer, RouteEntity route) {
		...
		return l == null || l.isEmpty() ? asList(routing(customer, route)) :
			routings(new ArrayList<>(l), customer, route);
	}
	...
	private void setItemFamilyAndNotDiscountedValue() {
		...
		List<ItemEntity> items =
			stream(list.spliterator(), false).map(i -> setItemFamilyAndNotDiscountedValue(i)).collect(toList());
	...
	private boolean isMismo(ItemEntity i) {
		List<String> mismos = Arrays.asList( //
			"104473", "104954", "104952", "105346", "105311", "105506", "105947", "105953", "105428", "104793", "104655",
			"106148", "106149",
			"105429", "105892", "105718", "105896", "105949", "105418", "105672", "105895", "105720", "104886", "105394",
			"104955", "104632",
	...
	private void updateBillingToCustomerWithUpdatedVendorIdThenDeleteCustomerWithObsoleteVendorId() {
		...
		for (int i = 0; i < obsoleteVendorIds.size(); i++)
			updateBillingToCustomerWithUpdatedVendorIdThenDeleteCustomerWithObsoleteVendorId(obsoleteVendorIds.get(i),
				updatedVendorIds.get(i));
	}

	private void updateBillingToCustomerWithUpdatedVendorIdThenDeleteCustomerWithObsoleteVendorId(Long obsoleteId,
	                                                                                              Long updatedId) {
	...
	private void updateBillingToCustomerWithUpdatedVendorIdThenDeleteCustomerWithObsoleteVendorId(List<BillableEntity>
		                                                                                              list,
	                                                                                              CustomerEntity c) {
	...
	private void givePickUpDiscountsToApprovedCustomers() {
		List<Long> vendorIds =
			Arrays.asList(13L, 21L, 26L, 29L, 35L, 504990464L, 503760889L, 503773439L, 504777816L, 504696211L);
