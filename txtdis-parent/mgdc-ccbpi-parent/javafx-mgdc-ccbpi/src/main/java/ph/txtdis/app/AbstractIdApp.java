...
public abstract class AbstractIdApp<AS extends Serviced<PK>, PK, ID> extends AbstractApp implements Launchable {
	...
	@Autowired
	protected AppField<String> lastModifiedByDisplay, remarksDisplay;

	@Autowired
	protected AppField<ZonedDateTime> lastModifiedOnDisplay;
	...
	protected BooleanExpression isPosted() {
		return lastModifiedOnDisplay.isNotEmpty();
	}
	...
	protected void updateLogNodes() {
		if (lastModifiedByDisplay != null) {
			lastModifiedByDisplay.setValue(((CreationTracked) service).getCreatedBy());
			lastModifiedOnDisplay.setValue(((CreationTracked) service).getCreatedOn());
		}
	}
}
