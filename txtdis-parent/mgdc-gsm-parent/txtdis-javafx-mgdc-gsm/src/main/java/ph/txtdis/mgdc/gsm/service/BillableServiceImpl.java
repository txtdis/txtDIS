...
public class BillableServiceImpl //
	...
	@Override
	public BillableDetail addPriceToDetail(boolean canAvailDiscount, BillableDetail detail, Customer customer, LocalDate orderDate) {
		detail.setPriceValue(getPrice(canAvailDiscount, detail, customer, orderDate));
		return detail;
	}

	private BigDecimal getPrice(boolean canAvailDiscount, BillableDetail detail, Customer customer, LocalDate orderDate) {
	...
	private Price getPrice(BillableDetail detail, String pricing, LocalDate orderDate) {
		...
		return i == null ? null
				: i.getPriceList().stream() //
						.filter(p -> p.getType().getName().equalsIgnoreCase(pricing) && isApprovedAndStartDateIsNotInTheFuture(p, orderDate))
