...
public class CustomerReceivableServiceImpl //
	...
	@Override
	public CustomerReceivableReport generateCustomerReceivableReport(Long customerId,
	                                                                 long minDaysOver,
	                                                                 long maxDaysOver) {
		return generateCustomerReceivablesFromInvoices(customerId, minDaysOver, maxDaysOver);
	}

	private CustomerReceivableReport generateCustomerReceivablesFromInvoices(Long customerId,
	                                                                         long minDaysOver,
	                                                                         long maxDaysOver) {
	...
	private void generateReceivableList(long min, long max, List<Billable> list) {
		receivables =
			list.stream().filter(b -> daysOver(b) >= min && daysOver(b) <= max).map(i -> toCustomerReceivable(i))
				.collect(toList());
	}
