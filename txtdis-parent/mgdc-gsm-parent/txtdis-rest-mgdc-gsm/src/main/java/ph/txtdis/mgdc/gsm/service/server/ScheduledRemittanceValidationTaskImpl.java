...
public class ScheduledRemittanceValidationTaskImpl //
	...
	public void voidAllUnvalidatedAfterPrescribedPeriodsSincePaymentAndCreationHaveBothExpired() {
		if (LocalDate.now().isBefore(LocalDate.of(2017, 6, 15)) || updatedRemittances)
			return;
		List<RemittanceEntity> l =
			service.findAllUnvalidatedAfterPrescribedPeriodSincePaidHasExpired(remittanceValidationPeriodInDays);
		if (l != null) {
			for (RemittanceEntity r : l)
				service.updatePaymentBasedOnValidation( //
					...
					r.getId().toString(), //
					"false", //
					"INVALID DUE TO NON-VALIDATION OVER " + remittanceValidationPeriodInDays +
						" DAYS SINCE PAYMENT AND/OR CREATION", //
					"SYSGEN", //
					toTimestampText(now()));
