...
public class ReceivingReportServiceImpl //
		...
		implements ReceivableService, ReceivingReportService, QtyToLoadOrderDetailItemMappingService {
	...
	protected BillableEntity firstEntity() {
		return receivingRepository.findFirstByCustomerTypeInAndRmaNullAndReceivingIdNotNullOrderByIdAsc(DELIVERED_ROUTES);
	}

	@Override
	protected BillableEntity nextEntity(Long id) {
		return receivingRepository.findFirstByCustomerTypeInAndRmaNullAndReceivingIdNotNullAndIdGreaterThanOrderByIdAsc(DELIVERED_ROUTES, id);
	}

	@Override
	protected BillableEntity lastEntity() {
		return receivingRepository.findFirstByCustomerTypeInAndRmaNullAndReceivingIdNotNullOrderByIdDesc(DELIVERED_ROUTES);
	}

	@Override
	protected BillableEntity previousEntity(Long id) {
		return receivingRepository.findFirstByCustomerTypeInAndRmaNullAndReceivingIdNotNullAndIdLessThanOrderByIdDesc(DELIVERED_ROUTES, id);
	}
